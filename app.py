...
st.set_page_config(page_title="GTIN/EAN Finder via Google CSE (Excel + Drive)", layout="wide")
st.title("GTIN/EAN Finder via Google CSE (Excel + Google Drive)")

# --- Secrets necesare ---
GOOGLE_API_KEY = st.secrets.get("GOOGLE_API_KEY")
GOOGLE_CSE_CX = st.secrets.get("GOOGLE_CSE_CX")
DRIVE_FOLDER_ID = st.secrets.get("DRIVE_FOLDER_ID")  # obligatoriu: ID folder partajat cu SA
OWNER_EMAIL = st.secrets.get("OWNER_EMAIL")          # optional: acordă acces direct după upload
...
drive_service, sa_email = get_drive_service()
...
def upload_to_drive(file_bytes: bytes,
                    filename: str,
                    mimetype: str = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"):
    if not DRIVE_FOLDER_ID:
        st.error("DRIVE_FOLDER_ID lipsește din Secrets. Setează ID-ul folderului partajat cu Service Account.")
    ...
    try:
        ...
        # opțional: acordă acces unui utilizator uman
        if OWNER_EMAIL:
            try:
        ...
        msg = getattr(e, "content", b"").decode(errors="ignore")
        st.error("Google Drive upload failed. Verifică: "
                 "1) DRIVE_FOLDER_ID corect, 2) folderul este partajat cu "
                 f"{sa_email} ca Editor. Detalii brute: {msg[:500]}")
        raise
...
st.sidebar.write("Drive folder:", DRIVE_FOLDER_ID or "NESETAT")
st.sidebar.write("Service Account:", sa_email)
st.sidebar.header("Quota")
st.sidebar.write("Requests in session:", st.session_state["request_count"])
st.sidebar.write("Est. daily free limit:", DAILY_LIMIT)
...
if uploaded:
    ...
    if st.button("Pornește căutarea EAN"):
        ...
        for idx, row in df.head(int(max_rows)).iterrows():
            ...
            # Skip dacă are deja EAN valid sau NOT_FOUND
        ...
        # Upload în Drive
        info = upload_to_drive(excel_data, "output_ean.xlsx")
        st.write("📂 Fișier salvat în Google Drive (folderul setat).")
        st.write("⬇️ Link direct:", info.get("webContentLink"))
        st.write("🌐 Vezi în Drive:", info.get("webViewLink"))
